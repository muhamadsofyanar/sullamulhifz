# Upload Manual ke GitHub — v4.9.0

`@phase 4.2` · `@phase 4.3` · `@phase 4.4` · `@phase 4.5` · `@phase 4.6` · `@phase 4.7` · `@phase 4.8` · `@phase 4.9`

Paket ini berisi perubahan runtime, UI, test, dokumentasi rilis, dan konfigurasi deployment. v4.9.0 tidak menambah migration baru. Ikuti urutan berikut agar Coolify tidak perlu diredeploy berulang kali.

## 1. Siapkan repository lokal

1. Backup repository lokal atau pastikan seluruh perubahan lama sudah di-commit.
2. Ekstrak ZIP v4.9.0.
...
```bash
git add -A
git commit -m "Release Sullamul Hifz v4.9.0 — Ruang Belajar Terpadu"
...
Pertahankan provider yang sudah dipakai sesuai `UPGRADE-V4.1.0.md`, lalu ikuti `UPGRADE-V4.9.0.md`. Pertahankan `APP_KEY` lama dan gunakan:
...
Sesudah aplikasi sehat, jalankan `php artisan sullam:verify-expansion-v480` dan `php artisan sullam:verify-learning-hub-v490`, lalu smoke test Ruang Belajar dengan minimal dua akun Personal untuk memastikan data tidak bercampur.
