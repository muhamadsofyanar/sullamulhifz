...
@section('content')
<div class="quran-v2-head">
    <div><span class="eyebrow">LATIHAN YANG MUDAH</span><h1>Latihan Al-Qur’an</h1><p>Pilih target atau surat, tentukan jumlah pengulangan, lalu tekan <strong>Mulai latihan</strong>.</p></div>
    @if(auth()->user()->hasAnyRole(['superadmin','institution_admin','head']))<a class="button secondary" href="{{ route('admin.quran-library.index') }}">Pustaka Qur’an</a>@endif
</div>
...
<div class="quran-admin-readiness"><span>Audio Juz 30: <strong>{{ $timingCount }}/564</strong></span><span>Preset: <strong>{{ $presets->count() }}</strong></span><span>Qari: <strong>{{ $sources->count() }}</strong></span></div>
@endif

@if($targets->isNotEmpty())
<section class="card quran-quick-section"><div class="section-head"><div><h2>Target dari guru</h2><p class="hint">Pilihan paling mudah untuk orang tua: cukup tekan Mulai.</p></div></div>
<div class="quran-quick-list">
@foreach($targets as $target)
<button type="button" class="quran-quick-card" data-load-target="{{ $target->id }}">
    <div class="quran-quick-icon">▶</div><div><small>{{ $target->student->full_name }}</small><strong>{{ $target->surah?->name_latin }} {{ $target->start_verse }}–{{ $target->end_verse }}</strong><span>{{ $target->rubu?->name ?? 'Target hafalan' }} · awal 10× per ayat</span></div><b>Mulai</b>
</button>
@endforeach
</div></section>
@endif
...
<section class="card quran-quick-section"><div class="section-head"><div><h2>Latihan siap pakai</h2><p class="hint">Tidak perlu mengatur ulang setiap kali.</p></div></div>
<div class="quran-simple-presets">@foreach($featuredPresets->take(6) as $preset)<button type="button" data-load-preset="{{ $preset->id }}"><span class="quran-simple-play">▶</span><div><strong>{{ $preset->title }}</strong><small>{{ $preset->repeat_count===0?'Tanpa batas':$preset->repeat_count.'×' }} · {{ $preset->repeat_scope==='each_item'?'setiap ayat':'seluruh bagian' }}</small></div></button>@endforeach</div>
</section>
@endif

<div class="quran-v2-layout">
<section class="card quran-simple-builder">
    <div class="section-head"><div><h2>Buat latihan sendiri</h2><p class="hint">Empat pilihan utama saja. Pengaturan lain disimpan di bagian lanjutan.</p></div></div>
    <form id="quran-builder" class="stack">
        <label class="senior-field">Qari<select name="source_id" required data-qari-source>@foreach($sources as $source)<option value="{{ $source->id }}" data-role="{{ data_get($source->metadata,'learning_role') }}" data-description="{{ data_get($source->metadata,'description') }}" @selected($defaultSource?->id===$source->id)>{{ $source->is_default?'⭐ ':'' }}{{ $source->reciter_name }}</option>@endforeach</select><small class="hint" data-qari-description>{{ data_get($defaultSource?->metadata,'description') }}</small></label>
...
<section class="card quran-player-card quran-player-v2" id="quran-player-card">
    <div class="quran-player-empty" data-player-empty><div class="quran-big-play">▶</div><h2>Siap menemani latihan</h2><p>Pilih target di atas atau buat latihan sendiri.</p></div>
    <div data-player-ready hidden>
        <div class="quran-player-v2-top"><div><small data-player-source>SUMBER AUDIO</small><h2 data-player-title>Latihan Al-Qur’an</h2><p data-player-summary></p></div><span class="badge" data-player-repeat></span></div>
        <div class="quran-now-playing-v2"><small>Sedang dibaca</small><strong data-player-current>—</strong><span data-player-progress>0/0</span><div class="quran-page-chip" data-player-page hidden>Halaman —</div></div>
        <audio id="quran-audio" preload="metadata"></audio>
        <div class="quran-main-controls"><button type="button" data-player-prev aria-label="Ayat sebelumnya">⏮</button><button type="button" class="quran-main-play" data-player-toggle>▶</button><button type="button" data-player-next aria-label="Ayat berikutnya">⏭</button></div>
        ...
        <button type="button" class="button ghost wide quran-stop-button" data-player-stop>Hentikan latihan</button>
