...
@section('content')
<div class="page-head">
    <div><span class="eyebrow">QARI TAHFIZH & PENGULANGAN</span><h1>Latihan Al-Qur’an</h1><p>Pilih Al-Husary untuk ketelitian tahfizh atau Al-Minshawi untuk murajaah; lalu atur ayat dan pengulangannya.</p></div>
...
<section class="card quran-builder-card">
    ...
    <form id="quran-builder" class="stack compact">
        <label>Sumber bacaan<select name="source_id" required data-qari-source>@foreach($sources as $source)<option value="{{ $source->id }}" data-role="{{ data_get($source->metadata,'learning_role') }}" data-description="{{ data_get($source->metadata,'description') }}" @selected($defaultSource?->id===$source->id)>{{ $source->is_default ? '⭐ ' : '' }}{{ $source->reciter_name }} — {{ data_get($source->metadata,'learning_role',$source->rewaya) }}</option>@endforeach</select><small class="hint" data-qari-description>{{ data_get($defaultSource?->metadata,'description') }}</small></label>
        <div class="form-grid">
...
const qariSource=form?.querySelector('[data-qari-source]');const showQari=()=>{const option=qariSource?.selectedOptions?.[0];const description=el('[data-qari-description]');if(description)description.textContent=option?.dataset.description||option?.dataset.role||''};qariSource?.addEventListener('change',showQari);showQari();
...
document.querySelectorAll('[data-load-preset]').forEach(btn=>btn.addEventListener('click',()=>load({preset_id:btn.dataset.loadPreset,source_id:form.source_id.value})));
