...
<section class="card"><div class="section-head"><div><h2>Antrean prioritas</h2><p class="hint">Murāja‘ah lewat jadwal didahulukan, lalu koreksi dan penguatan.</p></div><span class="v610-status {{ $priorityQueue->isEmpty()?'is-success':'is-warning' }}">{{ $priorityQueue->count() }} antrean</span></div><div class="v610-toolbar"><form method="get" action="{{ route('teacher.tahfizh.index') }}"><label class="sr-only" for="tahfizh-search">Cari santri</label><input id="tahfizh-search" type="search" name="q" value="{{ request('q') }}" placeholder="Cari nama atau kode santri"><button class="button secondary" type="submit">Cari</button>@if(request()->filled('q'))<a class="button ghost" href="{{ route('teacher.tahfizh.index') }}">Hapus pencarian</a>@endif</form></div><div class="v610-worklist">@forelse($priorityQueue as $item)<a class="v610-workitem" href="{{ route('teacher.tahfizh.student',$item['student']) }}"><div><h3>{{ $item['student']?->full_name }}</h3><small>{{ $item['reason'] }}</small></div><span class="v610-status {{ $item['priority']===1?'is-danger':'is-warning' }}">Buka setoran →</span></a>@empty<p class="empty">Tidak ada antrean mendesak. Anda dapat membuka daftar santri di bawah.</p>@endforelse</div></section>
<div class="grid two">
<section class="card">
