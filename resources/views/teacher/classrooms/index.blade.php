...
<div class="cards-grid">@forelse($assignments as $item)@php
    $target=$item->schoolClass ?: $item->learningGroup;
@endphp
<a class="class-card" href="{{ $item->class_id?route('teacher.classrooms.class',$item->class_id):route('teacher.classrooms.group',$item->learning_group_id) }}"><span class="eyebrow">{{ $item->program?->name }}</span><h2>{{ $target?->name }}</h2><p>{{ $item->class_id ? $target->activeEnrollments->count() : $target->activeMemberships->count() }} santri</p><span class="button ghost">Buka →</span></a>@empty<div class="card empty">Belum ada penugasan aktif.</div>@endforelse</div>
@endsection
