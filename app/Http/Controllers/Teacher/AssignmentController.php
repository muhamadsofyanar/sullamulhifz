...
class AssignmentController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $data = $request->validate([
            ...
            'assignment_type' => ['required', Rule::in(['reading','memorization','murajaah','listening','writing','family_activity','friday_development','general'])],
            'title' => ['required', 'string', 'max:190'],
            'target_text' => ['required', 'string'],
            'surah_id' => ['nullable', 'exists:quran_surahs,id'],
            'quran_audio_source_id' => ['nullable', Rule::exists('quran_audio_sources','id')->where('institution_id',$teacher->institution_id)],
            'start_verse' => ['nullable', 'integer', 'min:1'],
            'end_verse' => ['nullable', 'integer', 'gte:start_verse'],
            'repeat_count' => ['nullable', 'integer', Rule::in([1,3,5,10,20])],
            'repeat_mode' => ['nullable', Rule::in(['each_ayah','whole_selection'])],
        ...
        $assignment = DB::transaction(function () use ($data, $target, $teacherAssignment, $teacher, $year): Assignment {
            $assignment = Assignment::create([
                ...
                'quran_audio_source_id' => $data['quran_audio_source_id'] ?? null,
                ...
                'evidence_types' => $data['evidence_types'] ?? ['none'],
                'assigned_at' => now(),
            ...
            $studentIds = $data['target_type'] === 'class'
                ? $target->activeEnrollments()->pluck('student_id')
                : $target->activeMemberships()->pluck('student_id');
            foreach ($studentIds as $studentId) {
                AssignmentRecipient::create([
                    'assignment_id' => $assignment->id,
                    'student_id' => $studentId,
                    'recipient_source' => $data['target_type'],
                    'status' => 'assigned',
                ]);
            }
            return $assignment;
        ...
        return redirect()->route('teacher.assignments.show', $assignment)
            ->with('success', 'Tugas berhasil diterbitkan kepada '.$assignment->recipients()->count().' santri.');
    }
    ...
    public function review(Request $request, AssignmentSubmission $submission): RedirectResponse
    {
        ...
        $data = $request->validate([
            'review_status' => ['required', Rule::in(['accepted','revision_needed','completed'])],
            'teacher_feedback' => ['required', 'string'],
        ]);
        $submission->update([
            ...$data,
            'reviewed_by_teacher_id' => $request->user()->teacher->id,
            'reviewed_at' => now(),
        ]);
        $recipientStatus = $data['review_status'] === 'accepted' ? 'accepted' : $data['review_status'];
        $submission->recipient->update([
            'status' => $recipientStatus,
            'completed_at' => $data['review_status'] === 'completed' ? now() : null,
        ]);
